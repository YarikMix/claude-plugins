...
```bash
python3 scripts/selectel.py --cloud <имя> projects [--json]
```
...
- Первый сервисный пользователь аккаунта и его роли — создаётся только в панели, пароль
  показывается один раз. `403` при создании проекта через API — не хватает роли администратора
  аккаунта, выдать её тоже можно только в панели.
...
| `403` на создании проекта/пользователя | не хватает роли аккаунта (`member`/`iam.admin`) | роль администратора аккаунта в панели |
