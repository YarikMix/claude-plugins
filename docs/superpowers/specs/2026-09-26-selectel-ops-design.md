...
4. **Фаза 0 — pre-flight.** До любой работы `python3 scripts/selectel.py check [--cloud NAME | env]`.
   Как читать вывод: каждый шаг OK/FAIL с расшифровкой; при FAIL — таблица в разделе 9.
...
3. **DNS v2.** Зоны, фильтр, rrset, создание записи, формат имён, TTL, `comment`; правило
