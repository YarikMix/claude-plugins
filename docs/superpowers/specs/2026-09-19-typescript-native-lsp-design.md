...
- **Диагностика только по запросу.** `typescript-go` 7.0.2 не рассылает ошибки исходников через `textDocument/publishDiagnostics`: сам он публикует только диагностику `tsconfig.json`, а ошибки файла отдаёт в ответ на `textDocument/diagnostic`. Клиент, который умеет только принимать push, ошибок не увидит. Claude Code оказался именно таким — см. «Результаты проб».
...
Диагностика в Claude Code приезжает не сразу после правки, а с началом следующего хода пользователя, поэтому проба с той же ошибкой была оставлена вне курса до следующего хода. Сервер этот файл видел: `hover` по нему отвечал. С началом следующего хода блок диагностики не пришёл.

**Итог: плагин даёт навигацию и не даёт ошибок типов.** Claude Code принимает диагностику только через `publishDiagnostics`, а `typescript-go` 7.0.2 отдаёт ошибки исходников только по `textDocument/diagnostic`. Дополнительно подтверждены `goToDefinition` (`createFiberRoot` → `renderer-state.ts:10:17`), `documentSymbol` и `incomingCalls` (`clearContainer` ← `commitRoot`, вызов на `commit.ts:24:3`). README плагина описывает это как ограничение и отсылает к `tsc --noEmit`.

Конфигурацией плагина это не лечится: недостающее звено — запрос со стороны клиента. Закрывается либо на стороне Claude Code, либо прокси-процессом между клиентом и сервером, который сам опрашивает `textDocument/diagnostic` и переизлучает ответ как `publishDiagnostics`. Прокси — отдельная работа со своим дизайном; в этой он не делается.
